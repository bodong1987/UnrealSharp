...
namespace UnrealSharpTool.Core.CodeGen
{
    ...
    public class EnumTypeExporter : BaseTypeExporter
    {
        ...
        protected override bool ExportInternal()
        {
            ...
            {
                using ScopedCodeWriter EnumScope = new ScopedCodeWriter(Writer);
                {
                    int Padding = CalcPadLength();

                    foreach (var field in EnumType.Fields)
                    {
                        bool AnyValidComment = false;
                        ...
                        Writer.Write($"{field.Name!.PadRight(Padding)} = ({EnumType.UnderlyingTypeName}){value},");
                    }
                }
            }            
