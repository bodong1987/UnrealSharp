...
internal class StringPropertyProcessor : PropertyProcessor
{
    /// <summary>
    ...
    public override string GetCSharpTypeName(PropertyDefinition property, ELocalUsageScenarioType usage)
    {
        ...
        return usage.HasFlag(ELocalUsageScenarioType.GenericArgument) ? "string" : "string?";
    }
    ...
    /// <param name="usage">The usage.</param>
    /// <returns>System.String.</returns>
    public override string GetInteropGetValueText(PropertyDefinition property, string variable, string address, string offset, ELocalUsageScenarioType usage)
    {
        return usage.HasFlag(ELocalUsageScenarioType.StructView)
            ? $"{variable}{(variable.IsNotNullOrEmpty() ? " = " : "")}InteropUtils.GetStringView({address}, {offset})"
            : $"{variable}{(variable.IsNotNullOrEmpty() ? " = " : "")}InteropUtils.GetString({address}, {offset})";
    }
    ...
    /// <param name="usage">The usage.</param>
    /// <returns>System.String.</returns>
    public override string GetInteropSetValueText(PropertyDefinition property, string variable, string address, string offset, ELocalUsageScenarioType usage)
    {
        return usage.HasFlag(ELocalUsageScenarioType.StructView)
            ? $"InteropUtils.SetStringView({address}, {offset}, {variable})"
            : $"InteropUtils.SetString({address}, {offset}, {variable})";
    }
    ...
    public override string GetCppFunctionInvokeParameterName(PropertyDefinition property)
    {
        ...
        return $"US_STRING_TO_TCHAR({property.Name})";
    }
}