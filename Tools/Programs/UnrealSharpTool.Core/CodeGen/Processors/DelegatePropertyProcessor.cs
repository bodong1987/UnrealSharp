...
namespace UnrealSharpTool.Core.CodeGen.Processors
{
    ...
    class DelegatePropertyProcessor : CollectionPropertyProcessor
    {
        /// <summary>
        ...
        /// <returns>string.</returns>
        public override string GetCSharpTypeName(PropertyDefinition property, ELocalUsageScenarioType usage = ELocalUsageScenarioType.Common)
        {
            ...
            string parentCppName = property.Parent != null ? property.Parent.CppName! : "Z";
            string classScope = usage.HasFlag(ELocalUsageScenarioType.Interface) ? $"{parentCppName}." : "";
            if (property.IsDelegateProperty)
            {
                return $"TDelegate<{classScope}{parentCppName}_{property.Name}_Signature>?";
            }
            else
            {
                return $"TMulticastDelegate<{classScope}{parentCppName}_{property.Name}_Signature>?";
            }
        }
        ...
        public override bool HandleClassPropertyWrite(CodeWriter writer, PropertyDefinition property)
        {
            writer.Write("get");
            {
                ...
                string ownerTag = Context.SchemaType == EBindingSchemaType.CSharpBinding ? "this, " : "";
                string asTag = Context.SchemaType == EBindingSchemaType.CSharpBinding ? $" as F{property.Name}Delegate;" : "";
