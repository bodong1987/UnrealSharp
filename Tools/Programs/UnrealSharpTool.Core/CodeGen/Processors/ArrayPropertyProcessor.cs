...
internal class ArrayPropertyProcessor : CollectionPropertyProcessor
{
    /// <summary>
    ...
    public override string GetCSharpTypeName(PropertyDefinition property, ELocalUsageScenarioType usage)
    {
        ...
        {
            return usage.HasFlag(ELocalUsageScenarioType.ReturnValue) ? $"List<{property.GetInnerPropertyCSharpTypeName(Context, 0)}>?" : $"IList<{property.GetInnerPropertyCSharpTypeName(Context, 0)}>?";
        }
    ...
    )
    {
        ...
        Logger.EnsureNotNull(innerProcessor, "Failed find processor for array element.");
        ...
        return usage.HasFlag(ELocalUsageScenarioType.StructView)
            ? $"{variable}{(variable.IsNotNullOrEmpty() ? " = " : "")}new TArray<{property.GetInnerPropertyCSharpTypeName(Context, 0)}>({address}, {offset}, {constantsName}.{property.Name}_Property)"
            : $"{variable}{(variable.IsNotNullOrEmpty() ? " = " : "")}TArray<{property.GetInnerPropertyCSharpTypeName(Context, 0)}>.FromNative({address}, {offset}, {constantsName}.{property.Name}_Property)";
    }
    ...
    )
    {
        ...
        var innerPropcessor = Context.GetProcessor(property.InnerProperties[0]);
