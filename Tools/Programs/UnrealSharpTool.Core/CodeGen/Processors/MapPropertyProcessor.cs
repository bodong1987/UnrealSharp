...
namespace UnrealSharpTool.Core.CodeGen.Processors
{
    ...
    class MapPropertyProcessor : CollectionPropertyProcessor
    {
        /// <summary>
            ...
            ELocalUsageScenarioType usage)
        {
            ...
            PropertyProcessor? KeyProcessor = Context.GetProcessor(property.InnerProperties[0]);
            Logger.EnsureNotNull(KeyProcessor, "Failed find processor for map key element.");

            PropertyProcessor? ValueProcessor = Context.GetProcessor(property.InnerProperties[1]);
            ...
            {
                return $"{variable}{(variable.IsNotNullOrEmpty() ? " = " : "")}new TMap<{GetNotNullCSharpTypeName(property.GetInnerPropertyCSharpTypeName(Context, 0))}, {property.GetInnerPropertyCSharpTypeName(Context, 1)}>({address}, {offset}, {ConstantsName}.{property.Name}_Property)";
            }

            return $"{variable}{(variable.IsNotNullOrEmpty() ? " = " : "")}TMap<{GetNotNullCSharpTypeName(property.GetInnerPropertyCSharpTypeName(Context, 0))}, {property.GetInnerPropertyCSharpTypeName(Context, 1)}>.FromNative({address}, {offset}, {ConstantsName}.{property.Name}_Property)";
        }
