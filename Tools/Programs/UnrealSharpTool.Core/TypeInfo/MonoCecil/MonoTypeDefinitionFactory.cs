...
namespace UnrealSharpTool.Core.TypeInfo.MonoCecil
{
    ...
    /// </summary>
    internal class MonoTypeDefinitionFactory
    {
        #region Main Entry
        /// <summary>
        ...
        /// <param name="validator">The validator.</param>
        /// <returns>UnrealSharpTool.Core.TypeInfo.BaseTypeDefinition?.</returns>
        public static BaseTypeDefinition? SetupBaseTypeDefinition(BaseTypeDefinition baseTypeDefinition, TypeDefinition typeDefinition, IMonoTypeValidator validator)
        {
            ...
            baseTypeDefinition.PackageName = typeDefinition.Namespace.Substring("UnrealSharp.".Length).Replace(".Bindings.Defs", "").Replace(".", "/");
            ...
            if (mainAttr != null)
            {
                ...
                {
                    baseTypeDefinition.Flags = (UInt64)Convert.ChangeType(mainAttr.ConstructorArguments[0].Value, typeof(UInt64));
                }
                ...
                if (mainAttr.HasProperties)
                {
                    ...
                    {
                        int v;
                        int.TryParse(ExportFlagsProperty.Argument.Value.ToString(), out v);
        ...
        public static T LoadFlags<T>(IEnumerable<CustomAttribute> customAttributes) where T : Enum
        {
            foreach (var customAttribute in customAttributes)
            {
                if (customAttribute.IsCSharpImplementTypeAttribute())
                {
                    ...
                    {
                        UInt64 value = UInt64.Parse(customAttribute.ConstructorArguments[0].Value.ToString()!);
        ...
        {
            if(SetupBaseTypeDefinition(enumTypeDefinition, typeDefinition, validator) == null)
            {
                return null;
            }
        ...
        {
            bool isNativeBindingEnumType = typeDefinition.IsCSharpNativeBindingType();
            ...
            foreach (var field in typeDefinition.Fields)
            {
                ...
                EnumFieldDefinition value = new EnumFieldDefinition();
                value.Name = field.Name;
                value.Value = field.Constant != null ? (long)Convert.ChangeType(field.Constant.ToString()!, typeof(long)) : 0;
                ...
                MetaDefinition localDefine = new MetaDefinition();                
        ...
        /// <returns>System.Nullable&lt;BaseTypeDefinition&gt;.</returns>
        public static BaseTypeDefinition? SetupStructTypeDefinition(StructTypeDefinition structTypeDefinition, TypeDefinition typeDefinition, IMonoTypeValidator validator, HashSet<TypeReference> checkedReferences)
        {
            ...
            if (SetupBaseTypeDefinition(structTypeDefinition, typeDefinition, validator) == null)
            {
                return null;
            }
        ...
        /// <summary>
        /// Adds the depend type definition.
        /// </summary>
        /// <param name="structTypeDefinition">The structure type definition.</param>
        /// <param name="typeReference">The type reference.</param>
        /// <param name="checkedReferences">The checked type definitions.</param>
        public static void AddDependTypeDefinition(StructTypeDefinition structTypeDefinition, TypeReference typeReference, HashSet<TypeReference> checkedReferences)
        {
            if (checkedReferences.Contains(typeReference))
            {
                return;
            }

            checkedReferences.Add(typeReference);
            ...
            if (typeDefinition != null && (typeDefinition.IsCSharpImplementType()||typeDefinition.IsCSharpBindingDefinitionType()))
            {
                ...
                structTypeDefinition.AddDependNamespace(@namespace);
            }
            ...
            foreach (var e in elements)
            {
                if (e != null)
                {
                    AddDependTypeDefinition(structTypeDefinition, e, checkedReferences);
                }
            }
        }
        ...
        private static void CheckActorComponentAttributeLimitation(
            ...
            IEnumerable<Mono.Cecil.CustomAttribute> customAttributes
            )
        {
        ...
        {
            bool isFunctionLibrary = typeDefinition.IsFunctionLibrary();
            ...
            CheckActorComponentAttributeLimitation(structTypeDefinition, typeDefinition, propertyType, memberType, validator, customAttributes);
            ...
            bool bIsInterfaceDef = typeDefinition.IsInterface;

            if (bIsInterfaceDef || customAttributes.FirstOrDefault(x => x.AttributeType.FullName == typeof(UPROPERTYAttribute).FullName) != null)
            {
                PropertyDefinition propertyInfo = NewPropertyDefinitionInternal(structTypeDefinition, memberType, checkedReferences, validator);
                ...
                AddDependTypeDefinition(structTypeDefinition, propertyType, checkedReferences);
            }
        }
        ...
        {
            MonoDefaultValueParser defaultValues = new MonoDefaultValueParser(typeDefinition);
        ...
        {
            HashSet<TypeReference> CheckedReferences = new HashSet<TypeReference>();
            if (SetupStructTypeDefinition(scriptStructTypeDefinition, typeDefinition, validator, CheckedReferences) == null)
            {
                return null;
            }
        ...
        public static BaseTypeDefinition? SetupClassTypeDefinition(ClassTypeDefinition classTypeDefinition, TypeDefinition typeDefinition, IMonoTypeValidator validator)
        {
            ...
            if (SetupStructTypeDefinition(classTypeDefinition, typeDefinition, validator, CheckedReferences) == null)
            {
                return null;
            }
            ...
            if (typeDefinition.IsClass)
            {
                ...
                AddDependTypeDefinition(classTypeDefinition, typeDefinition.BaseType, CheckedReferences);
            ...
            else
            {
                ...
                else if (typeDefinition.Interfaces.First().InterfaceType.Resolve().IsUnrealType())
                {
                    ...
                    AddDependTypeDefinition(classTypeDefinition, typeDefinition.Interfaces.First().InterfaceType, CheckedReferences);
            ...
            if (classTypeDefinition.Metas.TryGetMeta("Config", out string? config))
            {
                ...
                classTypeDefinition.Flags |= (UInt64)EClassFlags.Config;
            }
            ...
            foreach (var property in classTypeDefinition.Properties)
            {
                if (property.Metas.TryGetMeta("ReplicatedUsing", out string? ReplicatedUsingName))
                {
                    if (classTypeDefinition.Functions.Find(x => x.Name == ReplicatedUsingName) == null)
                    {
                        ...
                        TypeDefinitionSourceInfo info = default;
                        ...
                        else
                        {
                            ...
                            if (field != null)
                            {
                                info = MonoTypeDefinitionSourceInfoBuilder.Build(field, validator.DebugInformation!);
                            }
                            else
                            {
                                info = MonoTypeDefinitionSourceInfoBuilder.Build(typeDefinition.Properties.FirstOrDefault(x => x.Name == property.Name)!, validator.DebugInformation!);
                            }
                        }

                        // invalid implement
                        ErrorReport.ReportError(
                            ErrorCode.MisingUFunctionAttribute,
        ...
        public static void LoadFunctions(ClassTypeDefinition classTypeDefinition, TypeDefinition typeDefinition, IMonoTypeValidator validator, HashSet<TypeReference> checkedReferences)
        {
            ...
            foreach (var method in typeDefinition.Methods)
            {
                ...
                FunctionTypeDefinition functionInfo = NewFunctionTypeDefinition(classTypeDefinition, method, validator, checkedReferences);

                classTypeDefinition.Functions.Add(functionInfo);

                foreach (var dependNS in functionInfo.DependNamespaces)
                {
                    classTypeDefinition.AddDependNamespace(dependNS);
                }
            }
        }
        ...
        private static void CheckFunctionAvailable(MethodDefinition method, IMonoTypeValidator validator)
        {
            if (!method.IsUFunction())
            {
                ErrorReport.ReportError(ErrorCode.MisingUFunctionAttribute, $"Method {method} should add [UFUNCTION] or [UEVENT]", MonoTypeDefinitionSourceInfoBuilder.Build(method, validator.DebugInformation!));
            }
        ...
        public static void SetupMetaDefinition(MetaDefinition metaDefinition, IEnumerable<CustomAttribute> customAttributes)
        {
            foreach (var attr in customAttributes)
            {
                if (attr.AttributeType.FullName == typeof(UMETAAttribute).FullName && attr.HasConstructorArguments)
                {
                    ...
                    {
                        string Name = attr.ConstructorArguments[0].Value.ToString()!;
                        string Value = attr.ConstructorArguments[1].Value.ToString()!;
                ...
                else if (attr.AttributeType.FullName == typeof(ToolTipAttribute).FullName && attr.HasConstructorArguments)
                {
                    if (attr.ConstructorArguments.Count == 1)
                    {
                        string Tooltip = attr.ConstructorArguments[0].Value.ToString()!;
        ...
        private static UInt64 LoadExtraFlags(MetaDefinition metas)
        {
            UInt64 Flags = 0;

            if (metas.HasMeta("IsReplicated"))
            {
                Flags |= (UInt64)EPropertyFlags.Net;
            }

            if (metas.HasMeta("ReplicatedUsing"))
            {
                Flags |= (UInt64)EPropertyFlags.Net;
                Flags |= (UInt64)EPropertyFlags.RepNotify;
            }
        ...
        public static EReferenceType GetReferenceType(Mono.Cecil.TypeDefinition typeDefinition)
        {
            if (typeDefinition.IsCSharpBuiltInType())
            {
                return EReferenceType.BuiltInType;
            }
            else if (typeDefinition.IsCSharpNativeBindingType())
            {
                return EReferenceType.UnrealType;
            }

            return EReferenceType.UserType;
        }
        ...
        public static string? GetClassPath(Mono.Cecil.TypeDefinition typeDefinition)
        {
            ...
            else
            {
                ...
                return attr.ConstructorArguments[2].Value.ToString();
            }            
        }
        ...
        private static void LoadInnerProperties(PropertyDefinition propertyDefinition, TypeReference typeReference, string guidSource, HashSet<TypeReference> checkedReferences, IMonoTypeValidator validator)
        {
            ...
            if (typeReference is Mono.Cecil.GenericInstanceType type)
            {
                foreach (var genericArg in type.GenericArguments)
                {
                    Logger.EnsureNotNull(propertyDefinition.Parent);
                    PropertyDefinition property = NewPropertyDefinition(propertyDefinition.Parent, genericArg, genericArg.Name, guidSource + genericArg.FullName, null, checkedReferences, validator);
        ...
        /// <summary>
        /// Creates new propertydefinition.
        /// </summary>
        ...
        public static PropertyDefinition NewPropertyDefinition(StructTypeDefinition parent, TypeReference typeReference, string name, string guidSource, IEnumerable<Mono.Cecil.CustomAttribute>? customAttributes, HashSet<TypeReference> checkedReferences, IMonoTypeValidator validator)        
        {
            PropertyDefinition propertyDefinition = new PropertyDefinition();

            propertyDefinition.Parent = parent;
            ...
            {
                SetupMetaDefinition(propertyDefinition.Metas, customAttributes);

                propertyDefinition.Flags |= (UInt64)LoadFlags<EPropertyFlags>(customAttributes);

                var defaultValueAttribute = customAttributes.FirstOrDefault(x => x.AttributeType.FullName == typeof(DefaultValueTextAttribute).FullName);
        ...
        public static PropertyDefinition NewPropertyDefinition(StructTypeDefinition parent, Mono.Cecil.PropertyDefinition property, HashSet<TypeReference> checkedReferences, IMonoTypeValidator validator)
        {
            ...
            if (accessMark == MethodAttributes.Public)
            {
                propertyDefinition.Flags |= (ulong)EPropertyFlags.NativeAccessSpecifierPublic;
            }
            else if (accessMark == MethodAttributes.Family || accessMark == MethodAttributes.FamORAssem || accessMark == MethodAttributes.Assembly)
            {
        ...
        public static PropertyDefinition NewPropertyDefinition(StructTypeDefinition parent, FieldDefinition fieldDefinition, HashSet<TypeReference> checkedReferences, IMonoTypeValidator validator)
        {
            ...
            if (accessMark == FieldAttributes.Public)
            {
                propertyDefinition.Flags |= (ulong)EPropertyFlags.NativeAccessSpecifierPublic;
            }
            else if (accessMark == FieldAttributes.Family || accessMark == FieldAttributes.FamORAssem || accessMark == FieldAttributes.Assembly)
            {
                propertyDefinition.Flags |= (ulong)EPropertyFlags.Protected;
                propertyDefinition.Flags |= (ulong)EPropertyFlags.NativeAccessSpecifierProtected;
            }
        ...
        public static PropertyDefinition NewPropertyDefinition(StructTypeDefinition parent, MethodDefinition methodDefinition, HashSet<TypeReference> checkedReferences, IMonoTypeValidator validator)
        {
            ...
            propertyDefinition.Flags |= (UInt64)EPropertyFlags.ReturnParm;
        ...
        {
            var propertyDefinition = NewPropertyDefinition(parent, parameterDefinition.ParameterType, parameterDefinition.Name, parameterDefinition.Method.ToString() + ":" + parameterDefinition.ToString(), parameterDefinition.CustomAttributes, checkedReferences, validator);

            if (parameterDefinition.ParameterType.IsByReference)
            {
                propertyDefinition.Flags |= (UInt64)EPropertyFlags.ReferenceParm;
                propertyDefinition.Flags |= (UInt64)EPropertyFlags.OutParm;
            }
        ...
        /// <summary>
        /// Creates new propertydefinitioninternal.
        /// </summary>
        /// <param name="parent">The parent.</param>
        ...
        private static PropertyDefinition NewPropertyDefinitionInternal(StructTypeDefinition parent, MemberReference memberReference, HashSet<TypeReference> checkedReferences, IMonoTypeValidator validator)
        {
            ...
            {
                return NewPropertyDefinition(parent, pd, checkedReferences, validator);
            }
            else if(memberReference is Mono.Cecil.FieldDefinition fd)
            {
                return NewPropertyDefinition(parent, fd, checkedReferences, validator);
            }
        ...
        #region Functions
        /// <summary>
        /// Creates new functiontypedefinition.
        /// </summary>
        /// <param name="parent">The parent.</param>
        /// <param name="method">The method.</param>
        /// <param name="validator">The validator.</param>
        ...
        public static FunctionTypeDefinition NewFunctionTypeDefinition(StructTypeDefinition parent, Mono.Cecil.MethodDefinition method, IMonoTypeValidator validator, HashSet<TypeReference> checkedReferences)
        {
            FunctionTypeDefinition functionTypeDefinition = new FunctionTypeDefinition();

            functionTypeDefinition.Parent = parent;
            functionTypeDefinition.Name = method.Name;
            functionTypeDefinition.CppName = method.Name;
            functionTypeDefinition.Namespace = parent.Namespace;
            functionTypeDefinition.PackageName = parent.PackageName;
            functionTypeDefinition.ProjectName = parent.ProjectName;
            ...
            {
                PropertyDefinition propertyDefinition = NewPropertyDefinition(functionTypeDefinition, paramDefinition, checkedReferences, validator);
                Logger.EnsureNotNull(propertyDefinition.Parent);

                functionTypeDefinition.Properties.Add(propertyDefinition);

                AddDependTypeDefinition(parent, paramDefinition.ParameterType, checkedReferences);
            }

            if (method.ReturnType.FullName != typeof(void).FullName)
            {
                // have return value
                PropertyDefinition ReturnDefinition = NewPropertyDefinition(functionTypeDefinition, method, checkedReferences, validator);
                Logger.EnsureNotNull(ReturnDefinition.Parent);

                functionTypeDefinition.Properties.Add(ReturnDefinition);

                AddDependTypeDefinition(parent, method.ReturnType, checkedReferences);
            }

            functionTypeDefinition.Flags |= (UInt32)LoadFlags<EFunctionFlags>(method.CustomAttributes);

            if (method.CustomAttributes.FirstOrDefault(x => x.AttributeType.FullName == typeof(UEVENTAttribute).FullName) != null)
            {
                functionTypeDefinition.Flags |= (UInt32)EFunctionFlags.BlueprintEvent | (UInt32)EFunctionFlags.Event;
            }

            if (method.IsPublic)
            {
                functionTypeDefinition.Flags |= (UInt32)EFunctionFlags.Public;
            }
            else if (method.IsFamily)
            {
                functionTypeDefinition.Flags |= (UInt32)EFunctionFlags.Protected;
            }
            else if (method.IsPrivate)
            {
                functionTypeDefinition.Flags |= (UInt32)EFunctionFlags.Private;
            }

            functionTypeDefinition.IsOverrideFunction = (method.IsVirtual && method.IsReuseSlot) || method.IsNewSlot;

            functionTypeDefinition.Signature = method.GetMethodSignature();

            if (functionTypeDefinition.IsEvent)
            {
                string ImeplementMethodName = method.Name + "_Implementation";

                var ImplementMethod = method.DeclaringType.Methods.FirstOrDefault(x => x.Name == ImeplementMethodName);
            ...
            {
                functionTypeDefinition.Flags |= (UInt32)Replicates;

                if (functionTypeDefinition.Metas.TryGetMeta("IsReliable", out bool IsReliable) && IsReliable)
                {
                    functionTypeDefinition.Flags |= (UInt32)EFunctionFlags.NetReliable;
                }
            }
