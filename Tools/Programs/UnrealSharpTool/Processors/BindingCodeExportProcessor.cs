...
namespace UnrealSharpTool.Processors
{
    ...
    class BindingCodeExportProcessor : AbstractBaseWorkModeProcessor<BindingCodeExportOptions>
    {
        /// <summary>
        ...
        protected override int Process(BindingCodeExportOptions options)
        {
            ...
            ITypeDefinitionDocumentInitializeOptions? globalOptions = null;

            if (options.GenerateSourceType == BindingCodeGenerateSourceType.Assembly)
            {
                globalOptions = LoadExtraOptions<AssemblySearchOptions>();
            }
            else if (options.GenerateSourceType == BindingCodeGenerateSourceType.CSharpCode)
            {
                globalOptions = LoadExtraOptions<CSharpCodeBasedGenerateOptions>();
            }
            else
            {
                globalOptions = LoadExtraOptions<JsonDocBasedGenerateOptions>();
            }
            ...
            Logger.Log("Start Create TypeDefinitionDucment, Please wait...");            
            ...
            if (options.GenerateSourceType != BindingCodeGenerateSourceType.JsonDoc)
            {
                try
                {
                    string TempDocumentDirectory = Path.Combine(options.UnrealProjectDirectory!, "Intermediate/UnrealSharp");
                    ...
                    if (options.GenerateSourceType == BindingCodeGenerateSourceType.CSharpCode)
                    {
                        TempPath = Path.Combine(TempDocumentDirectory, options.InputPath.GetDirectoryName() + ".Bindings.Defs.tmp.json");
                    }
                    else
                    {
                        TempPath = Path.Combine(TempDocumentDirectory, options.InputPath.GetFileName() + ".tmp.json");
                    }
            ...
            Logger.Log("Create TypeDefinitionDucment Success, Start Export, Please wait...");
            ...
            {
                CppBindingExporter cppExporter = new CppBindingExporter(Context);
            ...
            CSharpBindingExporter exporter = new CSharpBindingExporter(Context);
            ...
            CSharpBindingDefinitionPlaceholdersExporter defExporter = new CSharpBindingDefinitionPlaceholdersExporter(Context);
